...
export class GithubProxyService {
  ...
  private validatePath(path: string): string {
    ...
    for (const segment of segments) {
      if (segment === '..') {
        this.logger.warn(`Potential path traversal attack detected! Path: "${path}"`);
   ...
   */
  async getRepoTree(repoName: string, rawPath: string): Promise<ProxiedRepoItem[]> {
    ...
    if (!res.ok) {
      if (res.status === 404) {
        throw new NotFoundException(`Path ${rawPath} not found in repository ${repoName}`);
      }
    ...
    if (!Array.isArray(items)) {
      throw new BadRequestException(`Requested path ${rawPath} is a file, not a directory tree.`);
    }
   ...
   */
  async getRepoFile(repoName: string, rawPath: string): Promise<ProxiedFileContent> {
    const path = this.validatePath(rawPath);
    const endpoint = `/repos/${this.api.username}/${repoName}/contents/${path}`;
    ...
    if (!res.ok) {
      if (res.status === 404) {
        throw new NotFoundException(`File ${rawPath} not found in repository ${repoName}`);
      }
      throw new Error(`GitHub API failed: ${res.statusText}`);
    }
  ...
  async getRepoMetadata(repoName: string): Promise<ProxiedRepoMetadata> {
    try {
      ...
      const info = await this.api.fetchJson<GithubRepoInfo>(`/repos/${this.api.username}/${repoName}`);

      // 2. Releases
      let releases: GithubRelease[] = [];
      try {
        const res = await this.api.fetchRaw(`/repos/${this.api.username}/${repoName}/releases?per_page=5`);
        if (res.ok) {
          releases = (await res.json()) as GithubRelease[];
        }
      } catch (e: unknown) {
        const errorMsg = e instanceof Error ? e.message : String(e);
        this.logger.warn(`Failed to fetch releases for ${repoName}: ${errorMsg}`);
      }

      // 3. Contributors
      let contributors: GithubContributor[] = [];
      try {
        const res = await this.api.fetchRaw(`/repos/${this.api.username}/${repoName}/contributors?per_page=10`);
        if (res.ok) {
          contributors = (await res.json()) as GithubContributor[];
        }
      } catch (e: unknown) {
        const errorMsg = e instanceof Error ? e.message : String(e);
        this.logger.warn(`Failed to fetch contributors for ${repoName}: ${errorMsg}`);
      }
      ...
      let totalCommits = 0;
      try {
        const res = await this.api.fetchRaw(`/repos/${this.api.username}/${repoName}/commits?per_page=15`);
        if (res.ok) {
          commits = (await res.json()) as GithubCommit[];
        }
        
        const statsRes = await this.api.fetchRaw(`/repos/${this.api.username}/${repoName}/stats/contributors`);
        if (statsRes.ok) {
        ...
        const errorMsg = e instanceof Error ? e.message : String(e);
        this.logger.warn(`Failed to fetch commits for ${repoName}: ${errorMsg}`);
      }

      // 5. Issues (filter out PRs)
      let issues: GithubIssue[] = [];
      try {
        const res = await this.api.fetchRaw(`/repos/${this.api.username}/${repoName}/issues?state=all&per_page=20`);
        if (res.ok) {
      ...
      try {
        const res = await this.api.fetchRaw(`/repos/${this.api.username}/${repoName}/pulls?state=all&per_page=15`);
        if (res.ok) {
      ...
      try {
        const res = await this.api.fetchRaw(`/repos/${this.api.username}/${repoName}/actions/runs?per_page=10`);
        if (res.ok) {
        ...
        const errorMsg = e instanceof Error ? e.message : String(e);
        this.logger.warn(`Failed to fetch workflow runs for ${repoName}: ${errorMsg}`);
      }
